...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://exchange.idamurni.de/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://exchange.idamurni.de/#/swap?inputCurrency=${
      token0Address?.toLowerCase() === '0x4c28f48448720e9000907bc2611f73022fdce1fa' ? 'ETH' : token0Address
    }&outputCurrency=${
      token1Address?.toLowerCase() === '0x4c28f48448720e9000907bc2611f73022fdce1fa' ? 'ETH' : token1Address
    }`
...
export function getMiningPoolLink(token0Address) {
  return `https://exchange.idamurni.de/#/uni/ETH/${token0Address}`
}

export function getUniswapAppLink(linkVariable) {
  let baseUniswapUrl = 'https://exchange.idamurni.de'
  if (!linkVariable) {
